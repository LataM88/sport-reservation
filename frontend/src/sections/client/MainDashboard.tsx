...
import { FaThLarge, FaFutbol, FaRunning, FaDumbbell, FaSwimmingPool } from "react-icons/fa";
    ...
    return (
        <div className={styles.container}>
            ...
            <Title className={styles.subtitle} level={4}>
                Znajdź najlepsze obiekty sportowe w Twojej okolicy i zarezerwuj termin.
            </Title>
                        ...
                        return (
                            <Col key={cat.label}>
                                ...
                                >
                                    ...
                                    <Paragraph
                                        className={styles.categoriesCardDetailsText}
                                        style={{ margin: 0, color: isActive ? "white" : undefined }}
                                    >
                                        {cat.label}
                                    </Paragraph>
                                </button>
                    ...
                    <div className={styles.skeletonWrapper}>
                        {[...Array(3)].map((_, i) => (
                            <div key={i} className={styles.skeletonCard}>
                                <Skeleton.Image active style={{ width: "100%", height: 220, borderRadius: 16 }} />
                            </div>
                        ))}
                    </div>
                )}

                {!isLoading && !isError && visibleSections.map((section) => (
                    <FacilitySection
                        key={section.key}
                        title={section.title}
